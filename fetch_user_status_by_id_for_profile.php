...
if(mysqli_num_rows($result4)>0) {
	...
	while($row4 = $result4->fetch_assoc()) {
		...
		$syn .= 'src="'.$imgpath.$img.'"></div><span>'.$user_name.'</span></div><div id="rightside"><button class="btn2 edit-status-in-profile" id="'.$row4['status_id'].'">Edit</button><button class="btn2 view_pro_status_delete_btn" id="'.$row4['status_id'].'">Delete</button></div></div><div id="content"><ul><li><span>Time </span><span id="status-time">'.$row4["st"].' to '.$row4["et"].'</span></li><li><span>Subjects</span><span id="status-subs';
